...
export async function priceRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get<{ Querystring: PriceCompareQuery }>(
    '/price/compare',
    {
      schema: {
        tags: ['Price'],
        ...
        description:
          'Compares prices across multiple exchanges and identifies arbitrage opportunities.',
        querystring: {
          type: 'object',
          required: ['symbol'],
          properties: {
          ...
          200: {
            ...
            properties: {
              ...
              arbitrageOpportunity: {
                type: 'number',
                description: 'Best bid - best ask (positive = arb exists)',
              },
    ...
    async (
      request: FastifyRequest<{ Querystring: PriceCompareQuery }>,
      reply: FastifyReply
      ...
      const targetExchanges: SupportedExchange[] = exchangesParam
        ? (exchangesParam
            .split(',')
            .filter(e => allExchanges.includes(e as SupportedExchange)) as SupportedExchange[])
        : allExchanges;
      ...
      const arbitragePercent =
        bestAskPrice.ask > 0 ? (arbitrageOpportunity / bestAskPrice.ask) * 100 : 0;
