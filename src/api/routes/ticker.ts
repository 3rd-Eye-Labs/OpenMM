...
export async function tickerRoutes(app: FastifyInstance): Promise<void> {
  app.get<{ Querystring: TickerQuery }>(
    '/ticker',
    {
      schema: {
        tags: ['Market Data'],
        ...
        description:
          'Returns current price, bid/ask, spread, and 24h statistics for a trading pair on a specific exchange.',
        querystring: {
          type: 'object',
    ...
    async (
      request: FastifyRequest<{ Querystring: TickerQuery }>,
      reply: FastifyReply
      ...
      if (!exchange || !symbol) {
        return reply
          .status(400)
          .send({ error: 'Missing required parameters: exchange and symbol' });
      }
      ...
      if (!validExchanges.includes(exchange)) {
        return reply
          .status(400)
          .send({ error: `Invalid exchange. Must be one of: ${validExchanges.join(', ')}` });
      }
