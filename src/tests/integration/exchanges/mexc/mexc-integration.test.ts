...
describe('MEXC Integration Tests', () => {
  ...
  beforeEach(() => {
    ...
    mexcUserStream = new MexcUserStream(
      (_endpoint: string, _params: Record<string, unknown>, _method: string) => 
        Promise.resolve({})
    );
  ...
  describe('MEXC Trading Workflow Integration', () => {
    test('Complete order lifecycle: connect → create → monitor → cancel', async () => {
      if (MEXC_TEST_CONFIG.testMode) {
        return;
      }
      ...
      try {
        ...
        const cancellationUpdate = orderUpdates.find(update => 
          update.id === orderId && update.status.match(/cancelled|canceled/)
        );
  ...
  describe('MEXC Real-time Data Pipeline', () => {
    test('Market data flow: connect → subscribe → receive → transform', async () => {
      ...
      try {
        ...
        const tickerSub = await mexcWebSocket.subscribeTicker(MEXC_TEST_CONFIG.testSymbol, (ticker: Ticker) => {
          tickerData.push(ticker);
        });
        expect(tickerSub).toBeDefined();

        const tradesSub = await mexcWebSocket.subscribeTrades(MEXC_TEST_CONFIG.testSymbol, (trade: Trade) => {
          tradeData.push(trade);
        });
        expect(tradesSub).toBeDefined();

        const orderbookSub = await mexcWebSocket.subscribeOrderBook(MEXC_TEST_CONFIG.testSymbol, (orderbook: OrderBook) => {
  ...
  describe('MEXC Authentication Integration', () => {
    test('Auth flow: credentials → signing → authenticated requests → user streams', async () => {
      if (MEXC_TEST_CONFIG.testMode) {
        return;
      }
  ...
  describe('MEXC Multi-Asset Trading', () => {
    test('Multi-symbol operations: INDY/USDT + BTC/USDT workflows', async () => {
      const symbols = [MEXC_TEST_CONFIG.testSymbol, MEXC_TEST_CONFIG.altSymbol];
      const marketData = new Map<string, { tickers: Ticker[], trades: Trade[], orderbooks: OrderBook[] }>();
      ...
      try {
        ...
        for (const symbol of symbols) {
          ...
          const orderbookSub = await mexcWebSocket.subscribeOrderBook(symbol, (orderbook: OrderBook) => {
