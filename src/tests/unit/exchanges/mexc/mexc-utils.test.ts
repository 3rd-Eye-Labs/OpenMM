...
describe('MexcUtils', () => {
  ...
  describe('transformOrderStatus', () => {
    it('should transform MEXC order statuses to OpenMM format', () => {
      expect(MexcUtils.transformOrderStatus('NEW')).toBe('open');
      expect(MexcUtils.transformOrderStatus('PARTIALLY_FILLED')).toBe('open');
      expect(MexcUtils.transformOrderStatus('FILLED')).toBe('filled');
      expect(MexcUtils.transformOrderStatus('CANCELED')).toBe('cancelled');
      expect(MexcUtils.transformOrderStatus('CANCELLED')).toBe('cancelled');
      expect(MexcUtils.transformOrderStatus('REJECTED')).toBe('rejected');
      expect(MexcUtils.transformOrderStatus('EXPIRED')).toBe('rejected');
    });

    it('should default unknown statuses to open', () => {
      expect(MexcUtils.transformOrderStatus('UNKNOWN_STATUS')).toBe('open');
    });
  });

  describe('transformOrder', () => {
    it('should transform MEXC order to OpenMM format', () => {
      const mexcOrder = {
        orderId: '123456789',
        symbol: 'BTCUSDT',
        type: 'LIMIT',
      ...
      const result = MexcUtils.transformOrder(mexcOrder);
  ...
  describe('transformTicker', () => {
    it('should transform MEXC ticker to OpenMM format', () => {
      const priceData = { symbol: 'BTCUSDT', price: '50000.00' };
      const statsData = {
        bidPrice: '49999.00',
        askPrice: '50001.00',
        volume: '1000.50'
      ...
      const result = MexcUtils.transformTicker(priceData, statsData);

      expect(result.symbol).toBe('BTC/USDT');
      expect(result.last).toBe(50000.00);
      expect(result.bid).toBe(49999.00);
      expect(result.ask).toBe(50001.00);
      expect(result.baseVolume).toBe(1000.50);
      expect(result.timestamp).toBeGreaterThan(0);
    });

    it('should work with price data only', () => {
      const priceData = { symbol: 'INDYUSDT', price: '0.12345' };

      const result = MexcUtils.transformTicker(priceData);

      expect(result.symbol).toBe('INDY/USDT');
      expect(result.last).toBe(0.12345);
      expect(result.bid).toBe(0.12345);
      expect(result.ask).toBe(0.12345);
      expect(result.baseVolume).toBe(0);
    });
  });

  describe('transformOrderBook', () => {
    it('should transform MEXC order book to OpenMM format', () => {
      const mexcOrderBook = {
        bids: [['49999.00', '1.5'], ['49998.00', '2.0']],
        asks: [['50001.00', '1.0'], ['50002.00', '0.5']]
      };

      const result = MexcUtils.transformOrderBook(mexcOrderBook, 'BTC/USDT');

      expect(result.symbol).toBe('BTC/USDT');
      expect(result.bids).toEqual([
        { price: 49999.00, amount: 1.5 },
        { price: 49998.00, amount: 2.0 }
      ]);
  ...
  describe('transformTrade', () => {
    it('should transform MEXC trade to OpenMM format', () => {
      const mexcTrade = {
        id: 123456789,
        price: '50000.00',
        qty: '1.5',
      ...
      const result = MexcUtils.transformTrade(mexcTrade, 'BTC/USDT');
    ...
    it('should handle isBuyerMaker = true as sell', () => {
      const mexcTrade = {
        tradeId: 987654321,
        price: '0.12345',
        quantity: '100',
      ...
      const result = MexcUtils.transformTrade(mexcTrade, 'INDY/USDT');
  ...
  describe('edge cases and error handling', () => {
    ...
    it('should handle transformOrder with missing required fields', () => {
      const mexcOrder = {
        type: 'LIMIT',
        side: 'BUY',
        symbol: 'BTCUSDT'
    ...
    it('should handle transformTicker with missing fields', () => {
      const priceData = { symbol: 'BTCUSDT' };
      
      const result = MexcUtils.transformTicker(priceData);

      expect(result.symbol).toBe('BTC/USDT');
      expect(result.last).toBeNaN();
      expect(result.bid).toBeNaN();
      expect(result.ask).toBeNaN();
      expect(result.baseVolume).toBe(0);
    });

    it('should handle transformTrade with missing id fields', () => {
      const mexcTrade = {
        price: '50000.00',
        qty: '1.5',
        time: 1638360000000,
