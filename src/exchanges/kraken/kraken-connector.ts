...
export class KrakenConnector extends BaseExchangeConnector {
  ...
  async getRecentTrades(symbol: string, count: number = 1000): Promise<Trade[]> {
    ...
    try {
      const response = await fetch(`${this.baseUrl}/0/public/Trades?pair=${krakenSymbol}&count=${count}`);
      const data = await response.json();
      ...
      const tradesData = Object.values(data.result)[0] as Array<[string, string, number, string, string, string]>;
  ...
  async connectWebSocket(): Promise<void> {
    throw new Error('WebSocket not implemented in REST-only integration');
  }
  ...
  async connectUserDataStream(): Promise<void> {
    throw new Error('User data stream not implemented in REST-only integration');
  }
